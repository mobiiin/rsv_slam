...
class extendedKalmanFilter():
    ...
    def __init__(self, x, P, F, H, Q, R):
        ...
        self.x = x # mean state vector
        self.x_ = np.zeros(6) # Predicted Value
        self.P = P # Covariance matrix (denoting error)
        self.F = F # State Transition Matrix 6*6
        self.H = H # State Transition Matrix from position to observation 3*6 known as Jacobian Matrix
        #self.h = h
        self.Q = Q # noise
    ...
    def update(self, obs, h_mat):
        ...
        S = self.R + self.H.dot(self.P).dot(self.H.T) # Total Error
